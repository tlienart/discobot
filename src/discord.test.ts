...
import { type Config, DiscordClient } from "./discord";
...
describe("DiscordClient", () => {
	...
	test("should skip recovery for invalid Snowflake IDs", async () => {
		...
		await (
			client as unknown as { recoverSessions: () => Promise<void> }
		).recoverSessions();

		expect(fetchSpy).not.toHaveBeenCalled();
		expect(
			client.getSessionManager().getChannelMapping().has("invalid-id"),
		).toBe(false);
	});
	...
	test("should handle /new command", async () => {
		const prepareSessionSpy = spyOn(
			...
			"prepareSession",
		).mockReturnValue(new OpenCodeAgent("test-session"));
	...
	test("should handle !mode shortcuts", async () => {
		...
		const prepareSessionSpy = spyOn(
			...
			"prepareSession",
		).mockReturnValue(mockAgent as unknown as OpenCodeAgent);
		spies.push(prepareSessionSpy);
		spies.push(startSpy);
