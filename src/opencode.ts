...
export class OpenCodeAgent extends EventEmitter implements Agent {
	...
	async start(prompt?: string) {
		...
		if (this.useSandbox && this.sbxName) {
			const sbxBin = join(process.cwd(), "sbx", "bin", "sbx");
			spawnArgs = [sbxBin, "exec", this.sbxName, "--", opencodeBinary, ...args];
			console.log(`[Agent] Sbx Spawning: ${spawnArgs.join(" ")}`);
		} else {
			const fullPath =
				opencodeBinary === "opencode"
					? "/opt/homebrew/bin/opencode"
					: opencodeBinary;
			spawnArgs = [fullPath, ...args];
