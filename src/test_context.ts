...
async function runOpencode(sessionId: string, prompt: string) {
  ...
  const proc = spawn(["/opt/homebrew/bin/opencode", "run", "--format", "json", "--session", sessionId, prompt], {
    stdout: "pipe",
    stderr: "pipe",
  });
...
async function testContext() {
  ...
  const proc1 = spawn(["/opt/homebrew/bin/opencode", "run", "--format", "json", "My name is Thibaut. Remember this."], {
    stdout: "pipe",
    stderr: "pipe",
  });
